...
test('translate simple off file to jscad code', function (t) {
  const inputPath = path.resolve(filesPath, 'off/cube.off')
...
test('deserialize simple off to cag/csg objects', function (t) {
  const inputPath = path.resolve(filesPath, 'off/cube.off')
  const inputFile = fs.readFileSync(inputPath, 'utf8')
